...
class Deleter:
    ...
    def delete_emails(self, **kwargs) -> None:
        ...
        """
        logging.info(
            "Deleting the senders from the list in self.senders emails from the gmail account."
        )
    ...
    def delete_emails_from_sender(self, sender: str) -> None:
        ...
        answer = input(
            "Are you sure you want to delete {} emails from sender: {}? (y/n)".format(
                len(email_ids), sender
            )
        )
        if answer == "y":
            ...
            logging.info(
                "Deleted {} emails from sender: {}".format(len(email_ids), sender)
            )
        else:
    ...
    def get_num_emails(self, num_emails) -> List[str]:
        ...
        logging.info(
            "Getting the last {} emails from the gmail account.".format(num_emails)
        )
        emails = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Deletes emails from the gmail account."
    )
    parser.add_argument(
        "--top_n_senders",
        type=str,
        nargs="+",
        help="The email addresses of the senders to delete emails from.",
    )
    parser.add_argument(
        "--num_emails",
        type=int,
        default=4000,
        help="The number of emails to check through.",
    )
    parser.add_argument(
        "--progress_bar", type=bool, help="Whether or not to display a progress bar."
    )
